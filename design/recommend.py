...
def oldget_chain(elem, num, process):
...
def get_chain(num, pos, process):
    if pos == 0:
        ...
        chain = get_chain(process[pos][num][2], pos-1, process)
        chain.append(process[pos][num][0])
        return chain



def oldpredict(m, count, s, A):
...
def predict(m, num, s, A):
    ...
    for i in range(m):
        ...
        for k in temp_line.keys():
            temp_line[k].sort(key=lambda x: x[1], reverse=True)
            temp_line[k] = temp_line[k][0:num]
            next_line.extend(temp_line[k])
        process.append(next_line)

    ans = sorted(process[-1], key=lambda b: b[1], reverse=True)
    process[-1] = ans
